...
export function App() {
  ...
  useEffect(() => {
    const enabledProviderIds = getEnabledProviderIds(providerConfigs);
    ...
    if (!areSameProviderSelection(selectedProviderIds, normalizedSelectedProviderIds)) {
      setSelectedProviderIds(normalizedSelectedProviderIds);
    }

    if (enabledProviderIds.length === 0) {
      setError('Enable providers and add API keys in Settings first.');
      return;
    }

    setError((previous) =>
      previous === 'Enable providers and add API keys in Settings first.' ? null : previous
    );
  }, [providerConfigs, selectedProviderIds]);
